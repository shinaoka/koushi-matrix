...
fn last_session_pointer_round_trip_redacts_debug_output() {
    ...
    let pointer = koushi_key::LastSessionPointer::new(id.clone());
    let json = pointer.to_json().unwrap();
    let restored = koushi_key::LastSessionPointer::from_json(&json).unwrap();
...
/// It is removed from this crate because koushi-key is now keyring-free.
