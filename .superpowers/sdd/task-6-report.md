...
Complete after the final scanner parsing hardening. The scanner rejects the
reviewed semantic-association, multiline-gate, negative-early-exit,
comment/string fabrication, batch-record, and cross-file-helper escape paths.
...
- This report — `docs: record final diagnostics scanner verification`

The scanner commit changes only
`apps/desktop/src/scripts/releaseScripts.test.ts`. The report commit changes
...
3. Applies conditional control-flow barriers before accepting a collector. A
   conditional collector is accepted only when its condition is also present
   in the stderr gate, as in the existing `dropped_queries > 0` runtime form.
...
5. Recognizes generic `record(make_diagnostic_event(stage))`, wrapper helpers,
   ...
   events.
6. Resolves module-qualified environment, structured, and stderr helpers across
   scanned files. This covers `unread_trace::enabled()` without a file/function
   allowlist.
7. Uses the generic private-data-free finding reason
...
helpers, lexical sanitization, direct/helper/loop/transformed mirrors, cfg
handling, line preservation, and runtime inventory:

```text
PASS  14 passed | 121 skipped (135 total)
```
...
Result: exit 0, 81 matches. Classification totals:
...
The totals are `15 + 51 + 14 + 1 = 81`. The runtime scanner assertion returned
...
PASS  event_cache_repair_diagnostic_runs_without_trace_environment
      outer 1 passed + env-unset child 1 passed; 423 filtered in each process
PASS  focused releaseScripts scanner suite — 14 passed, 121 skipped
PASS  full releaseScripts.test.ts — 135 passed
PASS  npm --prefix apps/desktop test — 732 passed across 46 files
...
PASS  exact inventory — 81 matches, classified 15/51/14/1
...
- Current direct, helper, loop, transformed, batch, early-exit, transitive,
  cross-file, cfg, line-preservation, and runtime-inventory forms are covered.
- The scanner commit contains one file; this report commit contains one file;
