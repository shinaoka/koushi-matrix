...
| desktop update control | `checkForDesktopUpdate` | native desktop updater command; no Core state admission contract |
...
| pure/native/observation | `getDiagnosticSnapshot`, `listSavedSessions`, `observeViewportSync`, `resolveComposerKeyAction`, `beginComposerDraftRendererGeneration`, `acquireComposerDraftLease`, `releaseComposerDraftLease`, `preparedUploadPreview`, `previewRoomAddress`, `sendReadReceipt`, `setFullyRead`, `setTyping`, `queryMentionCandidates`, `setRoomListProjection`, `subscribeReceiptReader`, `receiveReceiptReader`, `updateReceiptReaderWindow`, `observeReceiptReaderAvatars`, `ackReceiptReader`, `readReceiptReaderResource`, `closeReceiptReader` | existing typed value or `void`; no state snapshot and no fabricated generation |
| desktop adapter update lifecycle | `getDesktopUpdateState`, `downloadDesktopUpdate`, `restartToInstallDesktopUpdate` | typed Tauri adapter state/action; no Core snapshot or renderer-owned lifecycle |
