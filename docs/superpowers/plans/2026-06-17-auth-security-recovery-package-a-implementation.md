...
- #46 room-key import/export uses the Matrix key-export file format used by
  ...
  and do not introduce a product-specific JSON, archive, or wrapper file format.
...
test("security settings drive Rust-owned room-key transfer and secure backup state", async ({ page }) => {
  ...
  await page.getByLabel(/key export destination/i).fill("/tmp/koushi-export.txt");
  ...
  await page.getByLabel(/recovery key destination/i).fill("/tmp/koushi-recovery.txt");
