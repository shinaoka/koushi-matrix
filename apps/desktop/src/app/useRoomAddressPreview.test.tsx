...
test("returns only Rust previews for the current account and raw draft", async () => {
  ...
  await act(async () => pending[0]!({ localpart: "first", full_alias: "#first:example.invalid", error: null, server_name: "example.invalid" }));
  expect(result.current).toBeNull();
  await act(async () => pending[1]!({ localpart: "manual", full_alias: "#manual:example.invalid", error: null, server_name: "example.invalid" }));
