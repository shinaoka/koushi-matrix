...
fn withheld_code_from_sdk(
    content: matrix_sdk_base::crypto::types::events::room_key_withheld::RoomKeyWithheldContent,
...
pub async fn inbound_group_session_index0_summary(
    session: &MatrixClientSession,
    room_id: &str,
) -> Result<(usize, bool), MatrixRoomOperationError> {
    let room_id = matrix_sdk::ruma::RoomId::parse(room_id)
        .map_err(|_| MatrixRoomOperationError::InvalidRoomId)?;
    session
        .client()
        .encryption()
        .inbound_group_session_index0_summary(room_id.as_ref())
        .await
        .map_err(MatrixRoomOperationError::from_sdk_error)
}
...
mod megolm_send_parity_tests {
    ...
    #[tokio::test]
    async fn stock_send_uses_one_standard_preshare() {
        let server = MatrixMockServer::new().await;
