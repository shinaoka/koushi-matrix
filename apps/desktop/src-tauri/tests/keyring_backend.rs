...
fn delete_missing_credential_is_success() {
    assert!(
        koushi_desktop::keyring_backend::map_delete_result(Err(keyring::Error::NoEntry)).is_ok()
    );
}
...
fn run_macos_temporary_keychain_round_trip() -> Result<(), String> {
    ...
    let store = CredentialStore::with_backend(
        format!("Kagome Keychain QA {unique}"),
        koushi_desktop::keyring_backend::KeyringCredentialBackend,
    );
    let secret = LocalUnlockSecret::generate();
    store
        .save(&key_id(), &secret)
