...
| `avatar_demand` | dedicated 1,500-reader source, eight visible plus eight prefetch images, scoped PNGs, disjoint window move/return, cache reopen and shared in-flight cancellation; requires `--core`, separate from `all` | `avatar_window_requests=ok` |
