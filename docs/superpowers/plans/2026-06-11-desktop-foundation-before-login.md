...
**Architecture:** Keep the live Matrix SDK boundary absent in this step. Promote the key-management spike into a production crate, add a fake backend crate that drives `matrix-desktop-state` and `matrix-desktop-search`, and add a static desktop shell that can be opened locally while the Tauri/React packaging is still pending.
...
1. Production `matrix-desktop-key` crate for local unlock-secret storage and key derivation.
2. `matrix-desktop-backend` fake runtime that owns the reducer state, fake rooms/messages, and verified search results.
...
- [x] Add `matrix-desktop-key` to the workspace by moving reusable logic from `spikes/key-management` into `crates/matrix-desktop-key`.
...
- [x] Add `matrix-desktop-backend` to the workspace as a no-network fake runtime around the reducer and search adapter.
