...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum WindowStatePersistenceGate {
    PreArm,
    Restoring {
...
fn window_event_is_geometry(event: &tauri::WindowEvent) -> bool {
    matches!(
        event,
...
fn persist_observed_window_geometry<R: tauri::Runtime>(
    window: &tauri::Window<R>,
) -> Result<(), String> {
...
fn persist_close_window_state_if_ready<R: tauri::Runtime>(
    window: &tauri::Window<R>,
