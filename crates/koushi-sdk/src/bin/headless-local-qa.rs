...
async fn run_authenticated(
    ...
    session_a: &koushi_sdk::MatrixClientSession,
    session_b: &koushi_sdk::MatrixClientSession,
    ...
    koushi_sdk::invite_user_to_room(session_a, &space_id, &user_b_id)
        .await
        .map_err(|error| format!("invite user to space failed: {error}"))?;
    koushi_sdk::invite_user_to_room(session_a, &room_id, &user_b_id)
        .await
...
async fn login(
    ...
    koushi_sdk::login_with_password(&request)
        .await
...
async fn join_with_retry(
    ...
    for _ in 0..POLL_ATTEMPTS {
        let _ = koushi_sdk::sync_once(session).await;
...
async fn wait_for_room_list_entries(
    ...
    room_id: &str,
    space_id: &str,
) -> Result<koushi_sdk::MatrixRoomListSnapshot, String> {
    ...
    for _ in 0..POLL_ATTEMPTS {
        ...
        match koushi_sdk::room_list_snapshot(session).await {
            Ok(snapshot)
...
async fn assert_can_send(
    ...
    let can_send = koushi_sdk::room_can_send_text_message(session, room_id)
        .await
...
async fn wait_for_message(
    ...
    for _ in 0..POLL_ATTEMPTS {
        ...
        match koushi_sdk::room_timeline_visible_items(
            session,
            room_id,
