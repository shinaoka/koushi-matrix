...
| pure/native/observation | `getDiagnosticSnapshot`, `listSavedSessions`, `observeViewportSync`, `resolveComposerKeyAction`, `beginComposerDraftRendererGeneration`, `acquireComposerDraftLease`, `releaseComposerDraftLease`, `preparedUploadPreview`, `previewRoomAddress`, `sendReadReceipt`, `setFullyRead`, `setTyping`, `queryMentionCandidates`, `setRoomListProjection`, `subscribeReceiptReader`, `receiveReceiptReader`, `updateReceiptReaderWindow`, `ackReceiptReader`, `readReceiptReaderResource`, `closeReceiptReader` | existing typed value or `void`; no state snapshot and no fabricated generation |
