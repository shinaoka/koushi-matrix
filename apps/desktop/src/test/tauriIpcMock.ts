...
function normalizeCommandResponse(command: string, value: unknown, generation = 0): unknown {
  ...
  if (
    (command === "export_history" || command === "retry_history_export") &&
    value &&
    typeof value === "object" &&
    "requestId" in value
  ) {
    // The Tauri command answers with the request it submitted and its admission.
    return {
      kind: "submitted",
      requestId: (value as { requestId: number }).requestId,
      admission: { protocolVersion: 1, admittedGeneration: generation }
    };
  }
