...
pub(crate) fn handle_user_profiles_updated(
    ...
    // a wholesale profile replacement (e.g. every sync batch) does not reset
    // counterpart-profile avatars to NotRequested and cause DM room avatar flicker.
    let known = super::avatar::collect_known_avatar_thumbnails(state, /*include_invites*/ false);
    ...
    state.profile.users = profiles_map;
    crate::state::refresh_profile_user_display_projection(
        &mut state.profile,
        own_user_id.as_deref(),
    );
