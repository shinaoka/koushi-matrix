...
own receipt after send success and selects the newest canonically provable
position among queried, current, and requested IDs. A stale SDK cache cannot
delay a successful forward acknowledgement, and a stale viewport request cannot
regress a newer multi-device boundary. The tracker clears counted events through
the authoritative acknowledged event in canonical order and emits a reliable
